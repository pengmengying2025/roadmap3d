...
// Skip with LAYERGRAPH_SKIP_SMOKE=1 (e.g. on machines without a browser).
...
test('cli test renders key frames of examples/deps', { skip: process.env.LAYERGRAPH_SKIP_SMOKE ? 'LAYERGRAPH_SKIP_SMOKE set' : false }, () => {
  const out = path.join(ROOT, 'out', 'smoke'); fs.rmSync(out, { recursive: true, force: true });
  const r = spawnSync(process.execPath, [path.join(ROOT, 'src/cli.mjs'), 'test', path.join(ROOT, 'examples/deps/layergraph.json'), '-o', out, '--times', '0.5,9.9'], { encoding: 'utf8', timeout: 180000 });
